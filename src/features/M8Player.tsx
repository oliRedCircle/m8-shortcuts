...
const isLeft = (frame: number) => !!(frame & 0b10000000)
const isUp = (frame: number) => !!(frame & 0b01000000)
const isDown = (frame: number) => !!(frame & 0b00100000)
const isShift = (frame: number) => !!(frame & 0b00010000)
const isPlay = (frame: number) => !!(frame & 0b00001000)
const isRight = (frame: number) => !!(frame & 0b00000100)
...
const SvgComponent: FC<{
  ...
  useEffect(() => {
    ...
    const onAnimationFrame = () => {
      ...
      updateButtonClass(buttonRight, isRight(frame[1]))
      updateButtonClass(buttonUp, isUp(frame[1]))
      updateButtonClass(buttonDown, isDown(frame[1]))
      updateButtonClass(buttonOpt, isOpt(frame[1]))
      updateButtonClass(buttonEdit, isEdit(frame[1]))
      updateButtonClass(buttonShift, isShift(frame[1]))
      updateButtonClass(buttonPlay, isPlay(frame[1]))
      num = requestAnimationFrame(onAnimationFrame)
    }
